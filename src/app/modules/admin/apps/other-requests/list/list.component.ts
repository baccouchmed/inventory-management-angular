...
export class ListComponent implements OnInit {
  ...
  getList(): void {
    ...
    this.productRequestService
      .getMyRequests(this.currentSize, this.currentPage, this.searchFilter, null)
      .subscribe(
        (res: Pagination<Product>) => {
          this.displayedList = res;
          this.displayedList.data = this.displayedList.data.map((val) => ({
            ...val,
            productsId: val.productsId.map((el) => ({
              ...el,
              quantityValidated: el.quantityValidated || el.quantityRequested,
            })),
          }));
  ...
  updateQuantityValidated(requestId: string, id: string, quantityRequested: number) {
    this.productRequestService
