...
export class FuseMockApiInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    // Try to get the internationalization handler
    const { handler, urlParams } = this._fuseMockApiService.findHandler(
      request.method.toUpperCase(),
      request.url,
    );

    // Pass through if the internationalization handler does not exist
    if (!handler) {
      return next.handle(request);
    }

    // Set the intercepted internationalization on the handler
